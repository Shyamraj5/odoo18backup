...
class ProductRecipeExcelReport(models.AbstractModel):
    _name = "report.product_recipe_excel"
    _description = "Excel report of Product Recipe"
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        worksheet.merge_range(
            "A1:D1", "FEED", format1d)
        worksheet.merge_range(
            "A2:D2", lines.default_code, format1a)
        worksheet.merge_range(
            "A3:D3", lines.name.upper(), format1a)
        category = [category.name for category in lines.meal_category_id]
        worksheet.merge_range(
        ...
        row = 7
        i = 0
        for line in lines.ingredients_line_ids: 
            i += 1
            worksheet.write("A%s" %row, str(int(i)), format1b)
            worksheet.write("B%s" %row, line.ingredient_id.name, format1b)
            worksheet.write("C%s" %row, line.quantity, format1b)
            worksheet.write("D%s" %row, line.product_uom.name, format1b)
            row += 1
        # merge range for notes heading
        line_length=len(lines.ingredients_line_ids)
