...
def html_to_escpos(html: str) -> bytes:
    ...
    def apply_style(text: str, style: dict) -> bytes:
        ...
        # Wrap lines to 42 chars for typical thermal printer width
        for line in textwrap.wrap(text, width=42):
            output += line.encode("ascii", "ignore") + b"\n"
        return output
    ...
    def parse_element(el, style=None):
        ...
        if isinstance(el, NavigableString):
            ...
            if "d-flex" in classes:
                ...
                line = " ".join(parts)
                result += apply_style(line, new_style)
                return
...
class KotPrintController(http.Controller):
    ...
    def get_flask_url(self):
        ...
        return config.flask_endpoint_url

    @http.route('/pos/print_kot', type='json', auth='public')
    def print_kot(self, receipt):
